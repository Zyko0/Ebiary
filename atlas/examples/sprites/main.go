...
func (g *Game) init() {
	...
	// Atlas creation
	g.atlas = atlas.New(512, 512, nil)
	g.atlasImage = g.atlas.NewImage(
		ebitenImage.Bounds().Dx(), ebitenImage.Bounds().Dy(),
	)
	g.atlasImage.DrawImage(ebitenImage, nil)
	g.drawList = &atlas.DrawList{}
}
...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	case DrawList:
		...
		for i := 0; i < g.sprites.num; i++ {
			s := g.sprites.sprites[i]
			dc.GeoM.Reset()
			dc.GeoM.Translate(-float64(w)/2, -float64(h)/2)
			dc.GeoM.Rotate(2 * math.Pi * float64(s.angle) / maxAngle)
			dc.GeoM.Translate(float64(w)/2, float64(h)/2)
			dc.GeoM.Translate(float64(s.x), float64(s.y))
			g.drawList.Add(dc)
		}
		// Flush to screen
		g.drawList.Flush(screen, &atlas.DrawOptions{})
	}
...
Press 'space' to change drawing mode`, ebiten.ActualTPS(), ebiten.ActualFPS(), g.sprites.num, g.mode)
